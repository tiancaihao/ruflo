...
# =============================================================================
# Step 2b: Patch statusline to handle nested swarm-state.json structure
# The statusline generated by init reads swarmState.updatedAt from root level,
# but MCP writes { swarms: { swarmId: { updatedAt, agents, ... } } }.
# Without this fix, the panel always shows 0 agents.
# =============================================================================
STATUSLINE_FILE=".claude/helpers/statusline.cjs"
if [ -f "$STATUSLINE_FILE" ]; then
  STATUSLINE_PATCH=$(safe_mktemp /tmp/ruflo-statusline-patch.XXXXXX.cjs)
  cat << 'ENDSTATUSLINEPATCH' > "$STATUSLINE_PATCH"
const fs = require("fs");
...
const newFunc = `function getSwarmStatus() {
  ...
  if (swarmState) {
    ...
    if (swarmState.swarms && typeof swarmState.swarms === 'object') {
      const entries = Object.entries(swarmState.swarms);
      for (const [, s] of entries) {
        if (s && s.status === 'running') { activeSwarm = s; break; }
      }
      if (!activeSwarm) {
        for (const [, s] of entries) {
          if (s && s.status !== 'terminated') { activeSwarm = s; }
        }
      }
    }
    const data = activeSwarm || swarmState;
    if (data && data.status === 'terminated' && activeSwarm) {
    ...
    const agentCount = (data.agents && data.agents.length) || data.agentCount || 0;
    if (age < staleThresholdMs || (agentCount > 0 && data.status === 'running')) {
      return {
        activeAgents: agentCount,
        maxAgents: data.maxAgents || CONFIG.maxAgents,
        coordinationActive: data.status === 'running' && age < staleThresholdMs,
      };
    }
  }
...
# =============================================================================
# Helper: download or copy a local agent file into the target directory
# =============================================================================
download_or_copy() {
  local filename="$1"
  local dest_dir="$2"
  local tmpfile
  tmpfile=$(safe_mktemp "/tmp/${filename}.XXXXXX")

  if [ -f "$SCRIPT_DIR/../src/${filename}" ]; then
      cp "$SCRIPT_DIR/../src/${filename}" "$tmpfile"
...
# =============================================================================
# Step 9: Install local-agent-loop.js
# =============================================================================
download_or_copy "local-agent-loop.js" "$MAIN_BASE_DIR" || warn "local-agent-loop.js unavailable (non-fatal)."

# =============================================================================
# Step 10: Install local-agent-tools.js
# =============================================================================
download_or_copy "local-agent-tools.js" "$MAIN_BASE_DIR" || warn "local-agent-tools.js unavailable (non-fatal)."
